...
<tr>
    <td scope="col" class="px-3 py-3">    <span >Service</span>    </td>

    <td scope="col" class="px-1 py-3 text-center">    <span>วันนี้</span>    </td>

    <td scope="col" class="px-1 py-3 text-center">    <span>เมื่อวาน</span>    </td>
...
<tbody class="text-lg">`,l=0;t.response.data.forEach(r=>{let d=0,c=0,x=0;r.status.forEach(i=>{i.status=="POD"?d+=i.items:i.status=="091"||i.status=="112"||i.status=="POD99"||i.status=="POD98"?x+=i.items:c+=i.items}),r.date&&(l++,a+=`<tr class="bg-white border-b ${e.ND>0?"text-green-800":""}">
...
<div
class="flex h-full items-baseline justify-center overflow-hidden break-all bg-red-800 text-white px-3"
style="width: ${n((x/(r.items||1)*100).toString(),0)}%">
</div>
`:""}
    ...
    <td class="text-xs md:text-sm text-center ${e.today_ND>0?"text-green-800":"text-gray-500"}">${r.items}</td>

    <td class="text-xs md:text-sm text-center ${c>0?l<=6?l<=3?"text-blue-500":"text-orange-500":"text-red-800":"text-gray-500"}">${c||"-"}</td>

    <td class="text-xs md:text-sm text-center ${d>0?"text-green-800":"text-gray-500"}">${d||"-"}</td>

    <td class="text-xs md:text-sm text-center ${x>0?"text-red-800":"text-gray-500"}">${x||"-"}</td>
...
<hr>
`,o.innerHTML=a,setTimeout(async function(){window.scrollTo({top:0,behavior:"smooth"})},50)}const q="3.7.4",Q=q,ee=typeof atob=="function",te=typeof btoa=="function",f=typeof Buffer=="function",k=typeof TextDecoder=="function"?new TextDecoder:void 0,A=typeof TextEncoder=="function"?new TextEncoder:void 0,re="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",b=Array.prototype.slice.call(re),y=(t=>{let e={};return t.forEach((s,o)=>e[s]=o),e})(b),se=/^(?:[A-Za-z\d+\/]{4})*?(?:[A-Za-z\d+\/]{2}(?:==)?|[A-Za-z\d+\/]{3}=?)?$/,p=String.fromCharCode.bind(String),B=typeof Uint8Array.from=="function"?Uint8Array.from.bind(Uint8Array):(t,e=s=>s)=>new Uint8Array(Array.prototype.slice.call(t,0).map(e)),M=t=>t.replace(/=/g,"").replace(/[+\/]/g,e=>e=="+"?"-":"_"),P=t=>t.replace(/[^A-Za-z0-9\+\/]/g,""),K=t=>{let e,s,o,a,l="";const r=t.length%3;for(let d=0;d<t.length;){if((s=t.charCodeAt(d++))>255||(o=t.charCodeAt(d++))>255||(a=t.charCodeAt(d++))>255)throw new TypeError("invalid character found");e=s<<16|o<<8|a,l+=b[e>>18&63]+b[e>>12&63]+b[e>>6&63]+b[e&63]}return r?l.slice(0,r-3)+"===".substring(r):l},_=te?t=>btoa(t):f?t=>Buffer.from(t,"binary").toString("base64"):K,v=f?t=>Buffer.from(t).toString("base64"):t=>{let s=[];for(let o=0,a=t.length;o<a;o+=4096)s.push(p.apply(null,t.subarray(o,o+4096)));return _(s.join(""))},h=(t,e=!1)=>e?M(v(t)):v(t),ne=t=>{if(t.length<2){var e=t.charCodeAt(0);return e<128?t:e<2048?p(192|e>>>6)+p(128|e&63):p(224|e>>>12&15)+p(128|e>>>6&63)+p(128|e&63)}else{var e=65536+(t.charCodeAt(0)-55296)*1024+(t.charCodeAt(1)-56320);return p(240|e>>>18&7)+p(128|e>>>12&63)+p(128|e>>>6&63)+p(128|e&63)}},ae=/[\uD800-\uDBFF][\uDC00-\uDFFFF]|[^\x00-\x7F]/g,I=t=>t.replace(ae,ne),L=f?t=>Buffer.from(t,"utf8").toString("base64"):A?t=>v(A.encode(t)):t=>_(I(t)),u=(t,e=!1)=>e?M(L(t)):L(t),O=t=>u(t,!0),oe=/[\xC0-\xDF][\x80-\xBF]|[\xE0-\xEF][\x80-\xBF]{2}|[\xF0-\xF7][\x80-\xBF]{3}/g,de=t=>{switch(t.length){case 4:var e=(7&t.charCodeAt(0))<<18|(63&t.charCodeAt(1))<<12|(63&t.charCodeAt(2))<<6|63&t.charCodeAt(3),s=e-65536;return p((s>>>10)+55296)+p((s&1023)+56320);case 3:return p((15&t.charCodeAt(0))<<12|(63&t.charCodeAt(1))<<6|63&t.charCodeAt(2));default:return p((31&t.charCodeAt(0))<<6|63&t.charCodeAt(1))}},R=t=>t.replace(oe,de),Z=t=>{if(t=t.replace(/\s+/g,""),!se.test(t))throw new TypeError("malformed base64.");t+="==".slice(2-(t.length&3));let e,s="",o,a;for(let l=0;l<t.length;)e=y[t.charAt(l++)]<<18|y[t.charAt(l++)]<<12|(o=y[t.charAt(l++)])<<6|(a=y[t.charAt(l++)]),s+=o===64?p(e>>16&255):a===64?p(e>>16&255,e>>8&255):p(e>>16&255,e>>8&255,e&255);return s},S=ee?t=>atob(P(t)):f?t=>Buffer.from(t,"base64").toString("binary"):Z,z=f?t=>B(Buffer.from(t,"base64")):t=>B(S(t),e=>e.charCodeAt(0)),H=t=>z(J(t)),le=f?t=>Buffer.from(t,"base64").toString("utf8"):k?t=>k.decode(z(t)):t=>R(S(t)),J=t=>P(t.replace(/[-_]/g,e=>e=="-"?"+":"/")),$=t=>le(J(t)),ie=t=>{if(typeof t!="string")return!1;const e=t.replace(/\s+/g,"").replace(/={0,2}$/,"");return!/[^\s0-9a-zA-Z\+/]/.test(e)||!/[^\s0-9a-zA-Z\-_]/.test(e)},X=t=>({value:t,enumerable:!1,writable:!0,configurable:!0}),V=function(){const t=(e,s)=>Object.defineProperty(String.prototype,e,X(s));t("fromBase64",function(){return $(this)}),t("toBase64",function(e){return u(this,e)}),t("toBase64URI",function(){return u(this,!0)}),t("toBase64URL",function(){return u(this,!0)}),t("toUint8Array",function(){return H(this)})},Y=function(){const t=(e,s)=>Object.defineProperty(Uint8Array.prototype,e,X(s));t("toBase64",function(e){return h(this,e)}),t("toBase64URI",function(){return h(this,!0)}),t("toBase64URL",function(){return h(this,!0)})},ce=()=>{V(),Y()},N={version:q,VERSION:Q,atob:S,atobPolyfill:Z,btoa:_,btoaPolyfill:K,fromBase64:$,toBase64:u,encode:u,encodeURI:O,encodeURL:O,utob:I,btou:R,decode:$,isValid:ie,fromUint8Array:h,toUint8Array:H,extendString:V,extendUint8Array:Y,extendBuiltins:ce};function C(t){var l;const e=document.querySelector("#response");if(t.data.length===0){e.innerHTML='<div class="text-pink-600 h-96 flex  items-center justify-center">ไม่พบข้อมูลที่ค้นหา</div>';return}let s="",o="",a='<span class="material-symbols-outlined">location_on</span>';(l=t.data)==null||l.forEach(r=>{var c;let d="";if(r.esaSurcharged=="1"&&(d=r.province=="ภูเก็ต"?'<span class="text-blue-600">พื้นที่ท่องเที่ยวพิเศษ</span>':"พื้นที่ห่างไกล"),r.masterPostalCodeCapacities&&(d='<span class="text-orange-600">พื้นที่ล่าช้า (Official)</span>'),r.warningMessage&&(d=r.warningMessage=="งดรับพัสดุไปยังปลายทางดังกล่าวชั่วคราว!!"?'<span class="text-red-600 KanitBold">พื้นที่ปิดรับพัสดุ!</span>':r.warningMessage),r.postalCode!=s){s=r.postalCode;let x=(c=t.report)==null?void 0:c.find(i=>i.postcode==s);o+=`<div class=" justify-between mt-2 pt-2 text-sm px-3 bg-gray-50 bg-opacity-80 flex md:hidden"><span>${r.province} ${r.postalCode}</span> ${d||"&nbsp;"}</div>`,o+='<hr class="my-2 border-green-700">',x.warning?a='<span class="material-symbols-outlined text-amber-900 animate-bounce">location_on</span>':a='<span class="material-symbols-outlined text-green-800">location_on</span>',r.inactive&&(a='<span class="material-symbols-outlined text-deep-orange-700 animate-pulse">wrong_location</span>')}o+=`<div class="rounded-md cursor-pointer hover:outline hover:outline-2 hover:outline-orange-900 hover:animate-pulse" data-target="${r.postalCode}" 
              data-detail="${N.encode(JSON.stringify([r.linehualRoute,r.postalCode,r.amphur]))}">
              <div class="flex justify-between items-center">
                ...
                `:""}
            </div>`}),e.innerHTML=o,document.querySelectorAll("[data-target]").forEach(r=>{r.addEventListener("click",async function(){const d=document.querySelector("#response");d.classList.add("loader");const c=this.dataset.target,x=this.dataset.detail??"",i={};i.store=JSON.parse(N.decode(x));const w=await g({action:"detail",value:c});i.response=w,G(i),d.classList.remove("loader")})})}var pe=["second","minute","hour","day","week","month","year"];function xe(t,e){if(e===0)return["just now","right now"];var s=pe[Math.floor(e/2)];return t>1&&(s+="s"),[t+" "+s+" ago","in "+t+" "+s]}var ue=["秒","分钟","小时","天","周","个月","年"];function ge(t,e){if(e===0)return["刚刚","片刻后"];var s=ue[~~(e/2)];return[t+" "+s+"前",t+" "+s+"后"]}var D={},W=function(t,e){D[t]=e},fe=function(t){return D[t]||D.en_US},m=[60,60,24,7,365/7/12,12];function U(t){return t instanceof Date?t:!isNaN(t)||/^\d+$/.test(t)?new Date(parseInt(t)):(t=(t||"").trim().replace(/\.\d+/,"").replace(/-/,"/").replace(/-/,"/").replace(/(\d)T(\d)/,"$1 $2").replace(/Z/," UTC").replace(/([+-]\d\d):?(\d\d)/," $1$2"),new Date(t))}function be(t,e){var s=t<0?1:0;t=Math.abs(t);for(var o=t,a=0;t>=m[a]&&a<m.length;a++)t/=m[a];return t=Math.floor(t),a*=2,t>(a===0?9:1)&&(a+=1),e(t,a,o)[s].replace("%s",t.toString())}function ye(t,e){var s=e?U(e):new Date;return(+s-+U(t))/1e3}var he=function(t,e,s){var o=ye(t,s&&s.relativeDate);return be(o,fe(e))};W("en_US",xe);W("zh_CN",ge);async function we(t){var l;const e=t.data.reduce((r,d)=>d.date?r+d.items:r+0,0),s=document.querySelector("#response");let o=`
...
<tbody class="text-lg">`,a=0;t.data.forEach(r=>{let d=0,c=0,x=0;r.status.forEach(i=>{i.status=="POD"?d+=i.items:i.status=="091"||i.status=="112"||i.status=="POD99"||i.status=="POD98"?x+=i.items:c+=i.items}),r.date&&(a++,o+=`
...
style="width: ${n((x/r.items*100).toString())}%">
</div>
</div>
</td>     
</tr>
    <tr class="border-b group/item group-hover/item:bg-gray-400">
    <td scope="row" class="pl-3 text-gray-900 whitespace-nowrap text-xs md:text-sm">${r.date}</td>
    <td class="text-center text-gray-900 text-xs md:text-sm">${n(r.items.toString())}</td>
    <td class="text-center text-green-800 text-xs md:text-sm">${n(d.toString())} <span class="text-xs block md:inline">(${n((d/r.items*100).toString())}%)</span></td>
    <td class="text-center text-xs md:text-sm ${a<=6?a<=3?"text-blue-800":"text-orange-500":"text-red-800"}">${n(c.toString())} <span class="text-xs block md:inline">(${n((c/r.items*100).toString())}%)</span></td> 
    <td class="text-center text-xs md:text-sm text-red-800">${n(x.toString())} <span class="text-xs block md:inline">(${n((x/r.items*100).toString())}%)</span></td> 
...
</div>`;window.onload=async()=>{const t=document.querySelector("#response");t.classList.add("loader");const e=await g({action:"official",value:null});C(e),t.classList.remove("loader")};var T;(T=document.querySelector("#SearchInput"))==null||T.addEventListener("keyup",async function(t){!this.value||t.key==="Backspace"||me(this.value)});var E;(E=document.querySelector("#home"))==null||E.addEventListener("click",async function(){const t=document.querySelector("#SearchInput"),e=document.querySelector("#response");e.classList.add("loader");const s=t.value?"search":"official",o=await g({action:s,value:t.value});C(o),e.classList.remove("loader")});var F;(F=document.querySelector("#setting"))==null||F.addEventListener("click",async function(){const t=document.querySelector("#response");t.classList.add("loader");const e=await g({action:"setting"});we(e),t.classList.remove("loader")});var j;function me(t){const e=document.querySelector("#response");e.classList.add("loader"),clearTimeout(j),j=setTimeout(async function(){const s=await g({action:"search",value:t});C(s),e.classList.remove("loader")},1e3)}
